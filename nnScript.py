...
def sigmoid(z):
    ...
    """
    return np.reciprocal(1.0 + np.exp(np.negative(z)))
...
def preprocess():
    ...
    selected_features = \
        [i for i in range(n_features) if not
         np.allclose(train_data.T[i], np.repeat(train_data.T[i][0], n_data))]

    print(len(selected_features))

    train_data = np.array([train_data.T[i] for i in selected_features]).T
    validation_data = np.array([validation_data.T[i] for i
                                in selected_features]).T
    test_data = np.array([test_data.T[i] for i in selected_features]).T
...
def nnObjFunction(params, *args):
    ...
    training_data = np.c_[training_data, np.ones(n_data)]

    # Feedforward pass.
    # At hidden layer.
    a = np.einsum('ji,ki->kj', w1, training_data)
    print('done: a ' + str(a.shape))
    z = sigmoid(a)
    print('done: z ' + str(z.shape))
    z = np.c_[z, np.ones(n_data)]
    print('done: add column to z')
    # At output layer.
    b = np.einsum('lj,ij->il', w2, z)
    print('done: b ' + str(b.shape))
    o = sigmoid(b)
    print('done: o ' + str(o.shape))
    # Computing objective function.
    # For 1-of-k encoding.
    def encode(n, k=n_class):
        row = np.zeros(k)
        row[int(n)] = 1
        return row
    # Computing y, the 1-of-k encoded matrix.
    y = np.array([encode(label) for label in training_label]) 
    print('done: y ' + str(y.shape))
    # J(W1, W2). Eq (7)
    J = y * np.log(o) + (1.0 - y) * np.log(1.0 - o)
    print('done: J ' + str(J.shape))

    Jw = (-1.0 / n_data) * np.sum(J)
    # Eq (15)
    obj_val = Jw + (lambdaval / (2.0 * n_data)) * \
              (np.sum(np.square(w1)) + np.sum(np.square(w2)))
    print('done: Jw ' + str(Jw.shape))

    error = o - y
    print('done: del ' + str(error.shape))
    J_w2 = np.einsum('il,ij->lj', error, z)
    J_w2 = (J_w2 + lambdaval * w2) / n_data
    print('done: J_w2 ' + str(J_w2.shape))

    del_w2 = np.einsum('il,lj->ij', error, w2[:,:-1])
    print('done: del_w2 ' + str(del_w2.shape))
    z1 = z[:,:-1]
    zz_del_w2 = (1 - z1) * z1 * del_w2
    J_w1 = np.einsum('ij,ip->jp', zz_del_w2, training_data)
    J_w1 = (J_w1 + lambdaval * w1) / n_data
    print('done: J_w1' + str(J_w1.shape))

    # Make sure you reshape the gradient matrices to a 1D array. for instance if your gradient matrices are grad_w1 and grad_w2
    # you would use code similar to the one below to create a flat array
    # obj_grad = np.concatenate((grad_w1.flatten(), grad_w2.flatten()),0)
    obj_grad = np.concatenate((J_w1.flatten(), J_w2.flatten()), 0)

    print('ERROR VALUE = {}'.format(obj_val))
...
def nnPredict(w1, w2, data):
    ...
    a = np.array([np.dot(w1, x) for x in data])    
    z = sigmoid(a)
    z = np.c_[z, np.ones(n_data)]
    b = np.array([np.dot(w2, zj) for zj in z])
    o = sigmoid(b)
...
def test():
    ...
    n_data, n_input = train_data.shape
    n_class = 10
    results = []

    for lambdaval in range(0, 61, 10):
        for n_hidden in range(20, 21, 4):
            initial_w1 = initializeWeights(n_input, n_hidden)
            initial_w2 = initializeWeights(n_hidden, n_class)

            initialWeights = np.concatenate((initial_w1.flatten(),
                                             initial_w2.flatten()), 0)
            args = (n_input, n_hidden, n_class, train_data,
                    train_label, lambdaval)
